              ...
              <motion.div key={active} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.4, ease: EASE }} style={{ width: '100%', height: '100%', position: 'absolute', inset: 0 }}>
                {!isPlaying ? (
                  <>
                    ...
                    {TESTIMONIALS[active].videoUrl && (
                      <motion.div
                        onClick={() => setIsPlaying(true)}
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.95 }}
                        style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 'clamp(60px, 10vw, 80px)', height: 'clamp(60px, 10vw, 80px)', borderRadius: '50%', background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', zIndex: 10 }}
                      >
                ...
                <div style={{ position: 'absolute', bottom: '1.5rem', left: '1.5rem', color: '#ffffff', fontFamily: 'var(--font-manrope)', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', pointerEvents: 'none' }}>
                  {TESTIMONIALS[active].videoUrl ? `Watch Film / 0${active + 1}` : `Testimonial / 0${active + 1}`}
                </div>
              </motion.div>
            </AnimatePresence>
